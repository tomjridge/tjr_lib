...
let mk fn = 
  ...
  object
    method fn=fn;
    method append=(fun s -> contents:=!contents^s);
    method close=(Tjr_file.write_string_to_file ~fn !contents)
  end

let unix = mk @@ dest_dir^"/msg_unix.ml"
let lwt = mk @@ dest_dir^"/msg_lwt.ml"
...
let _ = unix#append prelude; lwt#append prelude


let _ = unix#append "type file_descr
